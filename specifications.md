...
- If enable-login is true, then run launch_backend with --create-admin-user-with-password argument.
- If enable-login is true, then run launch_backend with --create-admin-user-with-password argument.
...
- Add an argument parser with arguments
    ...
    - One argument --create-default-user-with-password creates a AppUser instance with username = 'default'. This user is part of appuser_group and staging_group. An environment variable DEFAULT_USER_PWD must be set for this argument, which can be deleted after the user is created.
    
    - One argument --create-admin-user-with-password creates a User instance with username = 'admin' and is_staff = True. This user is part of admin. An environement variable ADMIN_USER_PWD, which can be deleted after the user is created.
...
- Add an argument parser with 
    ...
    - One argument --create-default-user-with-password creates a User instance with username = 'default'. This user is part of appuser_group and staging_group. A password string must be passed through an environment variable DEFAULT_USER_PWD. Do not allow plain text passwords as arguments for security reasons.
    
    - One argument --create-admin-user-with-password creates a User instance with username = 'admin' and is_staff = True. This user is part of admin. A password string must be passed through the environement variable ADMIN_USER_PWD. If an environement variable ADMIN_USER_PWD is found, the overwrite the argument input through the parser. Do not allow plain text passwords as arguments for security reasons.
    ...
    - Invoke the setup_user.py command with the --create-default-user-with-password argument and --create-default-user-with-password is passed in launch_backend.py.
    Make sure DEFAULT_USER_PWD is set in this case
    
    - Invoke the setup_user.py command with the --create-admin-user-with-password argument if and --create-admin-user-with-password is passed in launch_backend.py.
    Make sure ADMIN_USER_PWD is set in this place.
